                ...
                const int &spread[])
{
   ...
   for(int i=start; i<rates_total; i++)
   {
      ...
      // Condición de compra
      if(ema5Array[i] > ema10Array[i] && rsiArray[i] > 50.0 && stochKArray[i] > 50.0)
      {
         // Flecha debajo del mínimo
         UpSignalBuffer[i] = low[i] - 2*_Point;
      }
      // Condición de venta
      else if(ema5Array[i] < ema10Array[i] && rsiArray[i] < 50.0 && stochKArray[i] < 50.0)
      {
         // Flecha encima del máximo
